...
S3_BUCKET=debugroom-technical-academy-mattermost-gen2-bucket
#S3_BUCKET=${S3_BUCKET}
...
if [ "$1" = 'mattermost' ]; then
  ...
  then
    ...
    jq '.FileSettings.AmazonS3Region = "ap-northeast-1"' $MM_CONFIG > $MM_CONFIG.tmp && mv $MM_CONFIG.tmp $MM_CONFIG
#    jq '.FileSettings.Directory = "/mattermost/data/"' $MM_CONFIG > $MM_CONFIG.tmp && mv $MM_CONFIG.tmp $MM_CONFIG
    ...
    jq '.SqlSettings.DataSource = "postgres://mattermost:'${MM_PASSWORD}'@'${DB_HOST}':5432/'${MM_DBNAME}'?sslmode=disable&connect_timeout=10"' $MM_CONFIG > $MM_CONFIG.tmp && mv $MM_CONFIG.tmp $MM_CONFIG
